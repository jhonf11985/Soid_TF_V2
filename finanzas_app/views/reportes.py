...
"""
Vistas de reportes del módulo de Finanzas.
Todas las vistas filtran por tenant automáticamente.
"""
...
def reporte_resumen_mensual(request):
    ...
    if cuenta_id:
        ...
        cuenta_obj = CuentaFinanciera.objects.filter(tenant=tenant, pk=cuenta_id).first()
...
def reporte_movimientos_anulados(request):
    ...
    qs = MovimientoFinanciero.objects.filter(tenant=tenant).select_related(
...
def cxp_list(request):
    ...
    qs = CuentaPorPagar.objects.filter(tenant=tenant).select_related(
...
def cxp_detail(request, pk):
    ...
    obj = get_object_or_404(
        CuentaPorPagar.objects.filter(tenant=tenant).select_related(
            "proveedor", "categoria", "cuenta_sugerida"
        ),
        pk=pk
    )
...
def reporte_cxp_por_proveedor(request):
    ...
    qs = CuentaPorPagar.objects.filter(tenant=tenant).exclude(estado="cancelada")
...
def reporte_comparativo_anual(request):
    ...
    # ---- Query base ----
    qs_base = MovimientoFinanciero.objects.filter(tenant=tenant).exclude(estado="anulado")
    ...
    primer_movimiento = MovimientoFinanciero.objects.filter(tenant=tenant).order_by("fecha").first()
